...
Implémenter la diffusion d’erreur pour la alettisation d’images.

# Question 18 / reponse :

Pour répondre à cette question, nous avons donc créé une fonction nommé "mode_error_diffusion_palette" que voici : 
...
```
Cette méthode utilise donc un palette de couleur donné dès le début. Ensuite nous convertissons l'image en RGBA. On calcul ensuite la distance euclidienne entre deux couleurs puis on parcourt chaque pixel de l'image et on trouve la couleur la plus proche de la palette. Pour finir, on remplace le pixel par la couleur la plus procje et on calcul l'erreur que l'on diffuse aux voisins et ainsi de suite.
